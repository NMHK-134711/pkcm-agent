...
**싱글 6→3, 더블 6→4 둘 다 동작.** "완성"은 두 번 틀린 말이 됐다 — 바디프레스 계열의 스탯 오버라이드와 17종의 조건부 위력이 데이터에만 있고 엔진이 안 읽은 채 발견됐다(2026-09-01, 실제 게임과 대조하다 발견). 지금은 선언된 효과 전부를 캐스팅으로 검증하는 스윕 테스트가 회귀를 막는다 (`tests/test_effect_sweep.py`).
