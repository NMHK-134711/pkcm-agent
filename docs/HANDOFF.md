...
출전 로스터 특성 201종 중 198종, 도구 147종 전부가 실행된다. 테스트 530개, 코드 14.9k줄.
...
- **정책망이 없다.** 지금은 모든 합법 수를 균등하게 넓힌다. `SearchResult.distribution`이
  이미 방문 비율(= 탐색의 정책)을 내보내므로 **학습 타깃으로 바로 쓸 수 있다.**
...
붙인다. 지금까지: greedy가 random에 85%, 탐색이 random에 88% — **둘은 아직 구분되지
않는다.**
