...
- **정책망이 없다.** `policy.prior_over`가 조잡한 휴리스틱(위력×상성×자속)으로 그 자리에
  서 있고, **망이 생기면 함수 하나만 갈아끼우면 된다.** 그 자리가 승률을 53%→66%로
  올린 자리이므로, 제대로 된 사전확률이 얼마나 값어치가 있는지는 이미 측정됐다.
...
| 구성 | vs greedy | |
|---|---|---|
| DUCT, 깊이 6 | 53.1% ± 14.0% | 구분 안 됨 |
| DUCT + 롤아웃, 깊이 12 | 57.1% ± 18.3% | 구분 안 됨 |
| **PUCT 사전확률, 깊이 12** | **66.4% ± 8.5%** | **구분됨** |

**"구분 안 됨"을 "조금 낫다"로 읽지 말 것.** 처음 두 줄은 표본이 부족해서 아무 말도 못
하는 상태였고, 실제로 세 번째 변경 전까지는 정말 아무것도 나아지지 않았을 가능성이 크다.

**병목은 탐색량이 아니라 어디를 보느냐였다.** 결정당 엔진 스텝 3,600번을 쓰고도 한 턴짜리
계산기와 비겼는데, 사전확률을 넣자 이겼다. DUCT의 알려진 약점 — 양쪽이 균등 탐색하니
**트리 안의 상대가 초반에 거의 무작위로 두고**, 탐색이 실제보다 약한 상대를 상정한다.
