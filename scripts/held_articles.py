...
def main() -> int:
    ...
    lines = ["# Held pokesol articles",
             "",
             f"{len(rows)} of 241. The importer takes a party only when every "
             "one of the six is settled by the article itself, so these are "
             "the ones where something is missing rather than wrong.",
             ""]
    for row in rows:
