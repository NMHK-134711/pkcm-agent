...
코치는 실제 게임을 `runs/live/<날짜>.jsonl`에 한 줄씩 남깁니다(DESIGN.md §6 D). `begin`(양쪽 파티와 설정),
`open`(선발), `advice`(탐색의 추천과 방문분포), `turn`(우리 수, 상대 행동, 드러난 도구·특성, HP 보정, 이벤트,
그 뒤의 상태), `observe`, `end`(결과). 엔진이 승패를 본 게임은 `end`가 자동으로 붙고, 항복·시간·접속
끊김은 페이지 위쪽 **결과 → 기록** 버튼(또는 `/end?result=win|loss|draw&reason=...`)으로 직접 남깁니다.
`--log-dir`로 위치를 바꿀 수 있습니다. 기록이 실패해도 게임은 계속되고 오류만 화면에 뜹니다.
