...
출전 로스터 특성 201종 중 198종, 도구 147종 전부가 실행된다. 테스트 417개, 코드 14.5k줄.
...
- **PP 상한**: 챔피언스는 `(pp/5+1)*4`에 기본 PP 20 상한. 소스에서 확인했지만 게임에서
  한 번 대조해볼 가치가 있다.
- **더블 행동 순서의 동점 처리**: 우선도·스피드가 완전히 같을 때 무작위로 푼다.
  본편이 정말 무작위인지, 자리 순서 같은 결정적 규칙이 있는지 확인 안 됨.
- **파트너 조준**: `normal` 대상 기술로 자기 파트너를 때리는 걸 허용해뒀다 (Showdown이
  허용한다). 챔피언스 UI가 실제로 그 선택지를 주는지 확인 안 됨 — 안 준다면
  `state.move_targets`에서 `TARGET_ALLY`만 빼면 된다.
- **op.gg 스크랩의 마지막 항목**(플카열매)이 잘려 있어 설명·가격이 비어 있다. id는 정상.
- **특성 2종**(`eelevate`, `firemane`)은 공개된 한국어 명칭이 없어 영어로 표시된다.
- **학습기술은 전 세대 합집합이다.** 챔피언스 자체 학습기술표가 공개되면
