...
출전 로스터 특성 201종 중 198종, 도구 147종 전부가 실행된다. 테스트 448개, 코드 14.9k줄.
...
- **암컷 냐오닉스가 메가진화 가능한가.** 포케챔스 도감엔 메가 냐오닉스가 **하나**뿐이고
  (특성 트레이스, 수컷 기준), Showdown 챔피언스 모드엔 암수 둘 다 있다. 우리는 둘 다
  둔 상태 — 게임에서 암컷에게 냐오닉스나이트를 들려보면 바로 나온다.
